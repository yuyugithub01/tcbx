<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.id" placeholder="商品编号" clearable style="width: 160px;" class="filter-item" @keyup.enter.native="handleFilter"/>
      <el-select v-model="listQuery.brandId" class="filter-item" clearable placeholder="产地" style="width: 160px;" @change="changeSeries" @keyup.enter.native="handleFilter">
        <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"/>
      </el-select>
      <el-select v-model="listQuery.seriesId" class="filter-item" clearable placeholder="类型" style="width: 160px;" @keyup.enter.native="handleFilter">
    ...
    <el-table
      ...
      <el-table-column label="商品编号" prop="id" sortable="custom" align="center" min-width="105px">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="产地/类型/尺码" align="center" min-width="215px">
        <template slot-scope="scope">
    ...
    <el-dialog v-if="checkPermission(['admin'])" :visible.sync="dialogFormVisible" title="修改库存信息">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="产地 & 类型" prop="series">
          <el-cascader :options="options" v-model="temp.series" placeholder="产地/类型" style="width: 220px"/>
        </el-form-item>
        <el-form-item label="尺码" prop="type">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        seriesId: [
          { required: true, message: '请输入产地 & 类型' }
        ],
        type: [
          { required: true, message: '请输入商品尺码', trigger: 'blur' }
        ],
        color: [
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['库存编号', '产地', '类型', '尺码', '颜色', '进价', '售价', '入库时间', '状态']
